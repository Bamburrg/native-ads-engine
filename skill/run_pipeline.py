...
def _load_env_local():
    """Look for env in (1) skill dir alongside script, (2) parent project's .env.local."""
    here = Path(__file__).resolve().parent
    candidates = [here / ".env", here.parent / ".env.local"]
...
# Defaults — Convex URL is the shared dashboard so users without their own
# deployment land on https://native-ads-engine.vercel.app automatically.
os.environ.setdefault("NEXT_PUBLIC_CONVEX_URL", "https://zany-clownfish-30.convex.cloud")
...
def _fire_and_poll(prompt: str, cid: str, attempt: int) -> bytes | None:
    """Fire one kie.ai task and poll until success/failure/timeout. Return PNG bytes or None."""
    tid = kie_create(prompt)
    if tid is None:
        time.sleep(KIE_429_BACKOFF)
        tid = kie_create(prompt)
    if tid is None:
        print(f"  ! {cid} attempt {attempt}: kie 429 after backoff", flush=True)
        return None
    ...
    while time.time() < deadline:
        ...
        if state == "success":
            ...
            if not rj:
                print(f"  ! {cid} attempt {attempt}: success state but no resultJson", flush=True)
                return None
            rd = json.loads(rj)
            urls = rd.get("resultUrls") or rd.get("imageUrls") or []
            if not urls:
                print(f"  ! {cid} attempt {attempt}: no urls", flush=True)
                return None
            return http_download_bytes(urls[0])
        if state == "failed":
            print(f"  ! {cid} attempt {attempt}: kie failed: {info['data'].get('failMsg')}", flush=True)
            return None
    print(f"  ! {cid} attempt {attempt}: poll timeout", flush=True)
    return None


def synth_and_render(concept: dict, run_id: str, pass_n: int, lock: Lock, counters: dict):
    """Synth a single concept's render prompt, fire kie.ai (with one auto-retry), upload to Convex."""
    cid = concept["id"]
    try:
        ...
        # 3. Fire + poll, with one auto-retry on stall/failure
        png_bytes = _fire_and_poll(render_prompt, cid, attempt=1)
        if png_bytes is None:
            print(f"  ↻ {cid}: retrying once (fresh kie task)", flush=True)
            png_bytes = _fire_and_poll(render_prompt, cid, attempt=2)
        if png_bytes is None:
            print(f"  ✗ {cid}: failed after retry", flush=True)
            return
        ...
        # 4. Upload to Convex storage + record image row
        storage_id = convex_upload_bytes(png_bytes, "image/png")
        convex_call(
            "images:recordImage",
            {
                "runId": run_id,
